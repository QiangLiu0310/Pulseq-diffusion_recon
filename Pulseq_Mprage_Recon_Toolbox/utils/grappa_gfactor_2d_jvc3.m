function [ img_grappa, mask, mask_acs, image_weights, g_fnl ] = grappa_gfactor_2d_jvc3( kspace_sampled, kspace_acs, Rx, Ry, acs_size, kernel_size, lambda_percent, subs, delx, dely, weights_p, verbose )
...
if nargin < 7
    % percentage of sigma_min to use as regularizer
    lambda_percent = eps;
end
...
if lambda_percent
    ...
    lambda_tik = min( abs(s_inv) ) * lambda_percent / 100;
%     lambda_tik = max( abs(s_inv) ) * lambda_percent / 100;

    if verbose
        disp(['condition number: ', num2str(max(abs(s_inv)) / min(abs(s_inv)))])
        disp(['lambda: ', num2str(lambda_tik)])
    end
