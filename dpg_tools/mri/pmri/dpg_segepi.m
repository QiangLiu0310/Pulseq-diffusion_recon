function [F,I,N] = dpg_segepi( varargin );
%
% [F,I,N ] = dpg_segepi(Wk, Sk, Y, [opt args]);
% 
% DPG for multi-source segmented EPI.  DPG kernel is trained on multiple
% source images, one for each combination of {RO+,RO-,segment}
%
% opt args:
...
for cnt=1:2:length(vararg),
  ...
  elseif strcmp( vararg{cnt}, 'dks' ),
    dks = vararg{cnt+1};
  elseif strcmp( vararg{cnt}, 'dbgopt' ),
    dbgopt = vararg{cnt+1};
  elseif strcmp( vararg{cnt}, 'seg' ),
    seg = vararg{cnt+1};
  elseif strcmp( vararg{cnt}, 'segspace' ),
    segspace = vararg{cnt+1};
...
SkROp = zeros( Wm, Wn, ncoils ); SkROp(Y,:,:) = ROp;
SkROn = zeros( Wm, Wn, ncoils ); SkROn(Y,:,:) = ROn;
ROp = SkROp; 
ROn = SkROn; clear SkROp SkROn;
...
pt = regexp(kernel,'x');
nky = str2double(kernel(1:(pt-1)));           
nkx = str2double(kernel((pt+1):end)); % should be odd

indx3 = ceil((0-nkx)/2) -1 + (1:nkx); % [-2:2];

if iscell(ROp)
  if ( nky ~= ( length(ROp) + length(ROn))  )
    error([' Kernel "' kernel '" does not support the number of input ' ...
           'polarity data (' num2str(length(ROp)) '+' num2str(length(ROn)) ...
           ')      along ky (' num2str(nky) '). Kernel needs to be at least "' ...
           num2str(length(ROp)+length(ROn)) 'x' num2str(nkx) '".']);
...
%    nfc = length(f);
    ...
    %if ( length(indxpos) ~= nky/2 )
      ...
      for cnt2=1:length(indxpos)
        indxROp{cnt2} = indxpos(cnt2);
      end;
    %end;
    ...
    %if ( length(indxneg) ~= nky/2 )
      ...
      for cnt2=1:length(indxneg)
        indxROn{cnt2} = indxneg(cnt2);
      end;
    %end;
    ...
    for cnt2 = 1:length(plist{cnt}),
      ...
      for cnt3=1:length(indx3),
        ...
        if (dualks)
          ...
          if iscell(ROp)
            for cnt4=1:length(ROp)
              tmpSksub( indxROp{cnt4}, :, : ) = ROp{cnt4}( indx(indxROp{cnt4}),y0,: );
              tmpSksub( indxROn{cnt4}, :, : ) = ROn{cnt4}( indx(indxROn{cnt4}),y0,: );
            end;
          else
        ...
        A( rd*(cnt2-1) + (1:rd), (cnt3-1)*nky*ncoils + (1:nky*ncoils) ) = ...
            reshape( permute( tmpSksub, [ 2 1 3 ]), rd, size(Sk,3)*length(indx) );
    ...
    if (usenorm)
      ...
      % savend([ 'Araw_dpg_' pattern{cnt} '.nd' ], A);
      Araw = A;
      % A = tmult(A,diag(p_src),1);         % normalize by the power in each linsys
      A = A .* repmat(p_src(:),[1 size(A,2)]);
%      if (cnt==7), keyboard; end;
    ...
    AA = [A ]'*[A ];
    if (usenorm)
      %%% test Cauley's idea of "effective regularization rank" (ISMRM2015, p3619)
      % svdAA = svd(AA);
      % lams = logspace( log10(min(svdAA))-1, log10(median(svdAA)) + [ log10(max(svdAA)) - log10(median(svdAA)) ]/4,40);
      % for lll=1:length(lams),
      %   svdAA = svd(AA + lams(lll)*eye(size(AA)));
      %   etrunk(lll) = (median(svdAA) - min(svdAA))/(max(svdAA)-min(svdAA));
      %   etrunk_indx(lll) = sum( ( svdAA > etrunk(lll)*svdAA(1) ) );
      %   svdAAo(:,lll) = svdAA;
      % end;
      %%% the above is giving a regularization value that is a bit lower than desired, by eye

      if (length(chi)>1) 
        if isempty(chi)
          AAs = svd(AA);
          % chi = logspace( log10(min(AAs))-1, mean(log10([ max(AAs) min(AAs)])), 10 );
          chi = logspace( log10(min(AAs))-1, log10(max(AAs)), nlams );
        end;
        fprintf('chi: ');
        fprintf(' %f',log10(chi));
        fprintf('\n');
    ...
    for l=1:size(Sk,3),
      ...
      b = vec( squeeze(Sk( plist{cnt}, kx_indx, l )).' ); % for dual-kernel, this defaults
                                                          % to the target data
      if (usenorm)
      ...
      if (verbose>2),
        norg(:,l) = cgsolv( AA + chi(1)*eye(size(AA)), At*b, zeros(size(At,1),1), size(At,1) );
        pltcmplx( n(:,l,:), repmat( norg(:,l),[1 1 length(chi)]) );
        % pltcmplx( tmult(n(:,l,:),At',1), repmat(b,[1 1 length(chi)]) );
        keyboard;
      end;
    end;
    b1 = b2;
    ...
    if isstruct( dbgopt )
      if (cnt == 1)
        dAA = svd(AA);
        semilogy( dAA, dbgopt.pltopt );
        fprintf(' cond(AA) = %f\n', dAA(1)/dAA(dbgopt.cnum) );
      end;
    end;
