...
if (1),
  if isempty(phz)
    ...
    if (verbose)
      % figure; 
      tmp = [ ones(length(t2),1) t2(:) ]*y2;
      phdl = plot( t2-size(B,2)/2, unv(t2)-unv(t2(round(end/2))), 'bx-', t2-size(B,2)/2, tmp - tmp(round(end/2)),'k-.' );
...
%  B = diag( exp( j*y1(2)*((1:size(B,1))-1) ) ) * B;
  ...
  if nargout > 1,
    y_out = [ y2(2); exp(j*angle(x)) ];
    if (verbose)
      fprintf(' p = %f  %f + j %f\n',y_out(1,cnt), ...
      real(y_out(2)),imag(y_out(2)) );
    end;
  end;
