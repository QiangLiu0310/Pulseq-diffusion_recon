...
% shot 1 and shot 2
    path = ['D:\buda_data_21\20220106_183145_LQ_1_106\7']; 
    ...
    shot1=reshape(Prodata(:,:,:,:,1,:,:),[30 128 4 1 152]); % blip up acq
    shot2=reshape(Prodata(:,:,:,:,2,:,:),[30 128 4 1 152]); % blip down acq
    shot1= mrir_zeropad(shot1,[18 0 0 0 ],'pre');
    shot2= mrir_zeropad(shot2,[18 0 0 0 ],'post');
% A0 data
    path = ['D:\buda_data_21\20220106_183145_LQ_1_106\9']; %D:\IRIS_Kidney\20210805_182105_LQ_1_99\14
    fid = fopen([path,'\fid'],'r');
    Rawdata = fread(fid,'int32');
    fclose(fid);
    CompData = Rawdata(1:2:end)+1i.*Rawdata(2:2:end);
...
% A1 data
    path = ['D:\buda_data_21\20220106_183145_LQ_1_106\10']; %D:\IRIS_Kidney\20210805_182105_LQ_1_99\16
    fid = fopen([path,'\fid'],'r');
    Rawdata = fread(fid,'int32');
    fclose(fid);
    CompData = Rawdata(1:2:end)+1i.*Rawdata(2:2:end);
...
% A2 data (negative)
    path = ['D:\buda_data_21\20220106_183145_LQ_1_106\11']; 
    fid = fopen([path,'\fid'],'r');
    Rawdata = fread(fid,'int32');
    fclose(fid);
    CompData = Rawdata(1:2:end)+1i.*Rawdata(2:2:end);
...
% A3 data
    path = ['D:\buda_data_21\20220106_183145_LQ_1_106\12']; 
    fid = fopen([path,'\fid'],'r');
    Rawdata = fread(fid,'int32');
    fclose(fid);
    CompData = Rawdata(1:2:end)+1i.*Rawdata(2:2:end);
